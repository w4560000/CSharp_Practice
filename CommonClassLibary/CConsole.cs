...
{
    /// <summary>
    /// https://github.com/raminrahimzada/cConsole
    /// </summary>
    public class CConsole : IFormatProvider, ICustomFormatter
    {
        ...
        private static readonly CConsole This = new CConsole();

        static CConsole()
        {
            CustomFormatterType = typeof(ICustomFormatter);
            Colors = Enum.GetValues(typeof(ConsoleColor)).Cast<ConsoleColor>().ToDictionary(x => x.ToString().ToLowerInvariant(), x => x);
        }
